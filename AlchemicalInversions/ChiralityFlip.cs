...
//Credit to Kazyan for his amazing glyph flipper! This is all his code!
...
public static class ChiralityFlip
{
    public static Part cloneandflipPart(Part orig)
    {
        ...
        Part part;
        if (orig.method_1159() == Glyphs.Concatenation)
        { part = new Part(Glyphs.FlippyConcatenation, false); } //Disp left becomes disp right
        else
        { part = new Part(Glyphs.Concatenation, false); }
        ; //Disp right becomes disp left
    ...
    public static void SolutionEditorScreen_method_50(SolutionEditorScreen SES_self)
    {
        ...
        //1. Get all the parts in the selection that are Concatenation or FlippyConcatenation
        ...
        if (draggedParts.Count != 1) { class_238.field_1991.field_1872.method_28(0.5f); return; } // Only flip if we're dragging a single part; FTSIGCTU beep if multiple parts
        ...
        foreach (var draggedPart in draggedParts) // There should be only one draggedPart in draggedParts by this point in the code; I may or may not fix this later.
        {
            ...
            if (partType != Glyphs.Concatenation && partType != Glyphs.FlippyConcatenation) // or however I'm supposed to check this
            {
                //nothing to do; that's not a Glyph of Concatenation to flip
                continue;
            }
            else
            {
        ...
        class_238.field_1991.field_1877.method_28 (0.2f);  // 'sounds/ui_transition_back'
    }
}